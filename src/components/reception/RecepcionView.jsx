...
import { getLocalDateString, addDaysToDateString, formatDateAR } from '../../utils/dateUtils';
...
export const RecepcionView = () => {
  ...
  const [activeTab, setActiveTab] = useState('espera'); // 'espera' | 'citados' | 'historial'
  ...
  const handleToday = () => setCurrentDate(todayStr);

  // Turnos de la fecha seleccionada
  const turnosDeFecha = turnos.filter(t => t.fecha === currentDate);

  // Estadísticas de la fecha
  const totalFecha = turnosDeFecha.filter(t => t.estado !== 'CANCELADO').length;
  const enEsperaFecha = turnosDeFecha.filter(t => t.estado === 'EN_ESPERA').length;
  const enAtencionFecha = turnosDeFecha.filter(t => t.estado === 'EN_ATENCION').length;
  const atendidosFecha = turnosDeFecha.filter(t => t.estado === 'ATENDIDO').length;
  const pendientesLlegar = turnosDeFecha.filter(t => t.estado === 'PROGRAMADO').length;
  const totalRecaudadoCoseguros = turnosDeFecha
  ...
  const filteredTurnos = turnosDeFecha.filter(t => {
    if (activeTab === 'espera' && t.estado !== 'EN_ESPERA' && t.estado !== 'EN_ATENCION') return false;
    if (activeTab === 'citados' && t.estado !== 'PROGRAMADO' && t.estado !== 'CONFIRMADO') return false;
    if (activeTab === 'historial' && t.estado !== 'ATENDIDO' && t.estado !== 'CANCELADO' && t.estado !== 'NO_ASISTIO') return false;
    ...
    if (searchTerm.trim()) {
      ...
      const text = `${pac?.nombre || ''} ${pac?.apellido || ''} ${pac?.dni || ''} ${prof?.apellido || ''} ${t.codigo_reserva}`.toLowerCase();
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 space-y-5">
      {/* BARRA DE NAVEGACIÓN DE FECHA & CONTROL DE RECEPCIÓN */}
      <div className="bg-white p-4 rounded-3xl border border-slate-200 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3 flex-wrap">
          <div className="p-2.5 bg-tealmed-50 text-tealmed-700 rounded-2xl">
            <UserCheck className="w-6 h-6" />
          </div>
          <div>
            <h2 className="text-lg font-black text-slate-900 leading-tight">
              Módulo de Recepción & Sala de Espera
            </h2>
            <p className="text-xs text-slate-500">
              Control de ingresos físicos, llamado a consultorios y auditoría de atenciones.
            </p>
          </div>
        </div>

        {/* NAVEGADOR DE FECHA PARA AUDITORÍA Y FUTUROS */}
        <div className="flex items-center gap-2 bg-slate-50 p-1.5 rounded-2xl border border-slate-200">
          <button 
            onClick={handlePrevDay} 
            className="p-1.5 hover:bg-white text-slate-700 rounded-xl transition cursor-pointer"
            title="Día anterior (Ayer)"
          >
          ...
          <button 
            ...
            className={`px-3 py-1 text-xs font-black rounded-xl transition cursor-pointer ${
              isViewingToday ? 'bg-medical-600 text-white shadow-xs' : 'bg-white text-slate-700 hover:bg-slate-100'
            }`}
          >
            Hoy
          </button>
          ...
          <input 
            ...
            className="px-2.5 py-1 bg-white border border-slate-200 rounded-xl text-xs font-bold text-slate-800"
          />

          {!isViewingToday && (
            <span className="text-[11px] font-bold text-amber-800 bg-amber-50 px-2 py-0.5 rounded-lg border border-amber-200">
              {currentDate < todayStr ? 'Historial Pasado' : 'Próximas Llegadas'}
            </span>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        <div className="bg-white p-3.5 rounded-2xl border border-slate-200 shadow-2xs">
          <span className="text-[10px] font-black uppercase tracking-wider text-slate-400">Total Citados</span>
          <p className="text-xl font-black text-slate-900 mt-0.5">{totalFecha}</p>
        </div>
        ...
        <div className="bg-amber-50/70 p-3.5 rounded-2xl border border-amber-200 shadow-2xs">
          ...
          <p className="text-xl font-black text-amber-950 mt-0.5 flex items-center gap-1.5">
            {enEsperaFecha}
            {enEsperaFecha > 0 && <span className="w-2 h-2 rounded-full bg-amber-500 animate-ping inline-block" />}
          </p>
        </div>

        <div className="bg-purple-50/70 p-3.5 rounded-2xl border border-purple-200 shadow-2xs">
          ...
          <p className="text-xl font-black text-purple-950 mt-0.5">{enAtencionFecha}</p>
        </div>
        ...
        <div className="bg-emerald-50/70 p-3.5 rounded-2xl border border-emerald-200 shadow-2xs">
          ...
          <p className="text-xl font-black text-emerald-950 mt-0.5">{atendidosFecha}</p>
        </div>

        <div className="bg-slate-900 text-white p-3.5 rounded-2xl shadow-2xs col-span-2 sm:col-span-1 flex flex-col justify-between">
          <span className="text-[10px] font-black uppercase tracking-wider text-slate-400">Coseguros Cobrados</span>
      ...
      {/* SOLAPAS OPERATIVAS: SALA DE ESPERA / CITADOS / HISTORIAL */}
      <div className="bg-white p-3 sm:p-4 rounded-3xl border border-slate-200 shadow-xs space-y-3">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-3">
          <div className="flex items-center gap-1.5 p-1 bg-slate-100 rounded-2xl w-fit">
            {[
              { id: 'espera', label: `Sala de Espera (${enEsperaFecha + enAtencionFecha})`, icon: Users },
              { id: 'citados', label: `Citados / Llegadas (${pendientesLlegar})`, icon: Clock },
              { id: 'historial', label: `Histórico / Atendidos (${atendidosFecha})`, icon: History }
            ].map(tab => {
              const Icon = tab.icon;
              return (
                <button
          ...
          <div className="relative flex-1 max-w-xs">
            ...
            <input
              type="text"
              placeholder="Buscar por paciente, DNI, médico..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full pl-8 pr-3 py-1.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold focus:ring-2 focus:ring-medical-500"
            />
          </div>
        </div>
      </div>
